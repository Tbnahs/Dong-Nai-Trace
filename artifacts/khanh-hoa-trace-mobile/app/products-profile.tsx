...
function ProductEditor({ initial, hasTrace, onClose, onSave }: { initial: ProductForm; hasTrace: boolean; onClose: () => void; onSave: (form: ProductForm) => void }) {
  const colors = useColors();
...
export default function ProductsProfileScreen() {
  ...
  const saveProduct = async (form: ProductForm) => {
    ...
    const nextProduct: Product = { id: editor?.id ?? `p-${Date.now()}`, ...form, status: existing?.status ?? 'pending', traceCode: existing?.traceCode, gtin: form.gtin.trim() || undefined, lotCode: existing?.traceCode ? (form.lotCode.trim() || undefined) : undefined, updatedAt: new Date().toLocaleDateString('vi-VN') };
