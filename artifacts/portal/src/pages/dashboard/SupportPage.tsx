...
import { MessageSquare, Send, Clock, CheckCircle2, ChevronDown } from 'lucide-react';
              ...
              {tickets.map(t => (
                <div key={t.id} className="p-3 border border-gray-100 rounded-lg bg-gray-50 hover:bg-gray-100 cursor-pointer transition-colors">
                  <div className="flex justify-between items-start mb-1">
                    ...
                    {t.status === 'open' ? (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 flex items-center gap-1"><Clock className="w-3 h-3"/> Đang xử lý</span>
                    ) : (
                      <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-700 flex items-center gap-1"><CheckCircle2 className="w-3 h-3"/> Đã đóng</span>
                    )}
                  </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
            ...
            <form className="space-y-5">
              ...
              <div>
                ...
                <input type="text" className="w-full border border-gray-300 rounded-md p-2.5 focus:ring-[#1B2A6B] focus:border-[#1B2A6B]" placeholder="Ví dụ: Không thể tải lên chứng nhận VietGAP" />
              </div>

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Nội dung chi tiết</label>
                <textarea rows={5} className="w-full border border-gray-300 rounded-md p-2.5 focus:ring-[#1B2A6B] focus:border-[#1B2A6B]" placeholder="Mô tả chi tiết vấn đề bạn đang gặp phải..."></textarea>
              </div>

              <div className="flex justify-end pt-4">
                <button type="button" className="flex items-center gap-2 px-6 py-2.5 bg-[#E8650A] text-white font-bold rounded-md hover:bg-[#D55C08] transition-colors shadow-sm">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
        ...
        <div className="space-y-4">
          {[
            ...
            { q: 'Làm sao để được cấp mã định danh?', a: 'Sau khi hồ sơ tổ chức được phê duyệt (100% profile), chức năng "Yêu cầu cấp mã" sẽ được kích hoạt tại tab "Mã định danh".' },
            ...
            <div key={idx} className="border border-gray-200 rounded-lg overflow-hidden">
              <button className="w-full flex justify-between items-center p-4 bg-gray-50 hover:bg-gray-100 text-left font-semibold text-gray-800">
                {faq.q}
